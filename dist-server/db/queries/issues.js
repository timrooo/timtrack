...
export function getIssuesByPart(partId) {
    return getDb().prepare('SELECT * FROM issues WHERE part_id = ? ORDER BY created_at DESC').all(partId);
}
export function getIssuesByConstruction(constructionId) {
...
export function createIssue(data) {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).run(data.part_id, data.construction_id, data.type, data.comment, data.urgency, data.reported_by, data.raw_input ?? null);
