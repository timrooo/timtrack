...
export function seed(db: Database.Database): void {
  const seedAll = db.transaction(() => {
    ...
    for (const f of floorNames) {
      ...
      floorIds.push(res.lastInsertRowid as number)
    }
    ...
    for (const p of parts) {
      ...
      partIds.push(res.lastInsertRowid as number)
    }
    ...
    `).run(
      ...
      ws12Id,
      'paint',
      'Царапина на лицевой поверхности двери, требуется покраска',
      'medium',
      'Даниил К.',
      'дверь поцарапана нужна покраска',
      '2024-11-13 10:00'
    )
